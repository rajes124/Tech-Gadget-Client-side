...
<section className={`flex flex-col justify-center items-center min-h-screen pt-20 pb-12 text-center px-6 md:px-12 relative overflow-hidden ${
 ...
 {/* 3. Stats Section - Perfect Dark & Light Mode with Visible Icons */}
<section className="py-20 sm:py-24 px-6 md:px-12 bg-gradient-to-b from-blue-50/80 via-white to-blue-50/50 dark:from-indigo-950/60 dark:via-blue-950/40 dark:to-gray-900/80">
  <div className="max-w-6xl mx-auto text-center">
    <motion.h2 
      initial={{ opacity: 0, y: 30 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      className="text-4xl sm:text-5xl md:text-6xl font-black mb-6 bg-gradient-to-r from-blue-600 to-green-600 bg-clip-text text-transparent drop-shadow-2xl"
    >
      Trusted by 45K+ Businesses
    </motion.h2>
    <p className="text-lg sm:text-xl text-gray-700 dark:text-gray-300 mb-20 max-w-2xl mx-auto leading-relaxed">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 lg:gap-16 max-w-5xl mx-auto">
      {[
        ...
        { 
          icon: FaUsers, 
          value: stats.totalUsers.toLocaleString() || "45,000", 
          label: "Active Users", 
        ...
        { 
          icon: FaTruck, 
          value: `$${ (stats.totalExports || 2500).toLocaleString() }K`, 
          label: "Total Exports", 
        ...
        <motion.div
          ...
          className="group relative p-8 rounded-3xl bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-2xl hover:shadow-3xl transition-all duration-500"
        >
          {/* Glowing background on hover */}
          <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-green-500/10 rounded-3xl opacity-0 group-hover:opacity-100 blur-3xl transition-opacity duration-700 -z-10"></div>
          
          <div className={`text-6xl sm:text-7xl mb-8 ${stat.lightColor} dark:${stat.darkColor} drop-shadow-2xl group-hover:scale-110 group-hover:drop-shadow-glow transition-all duration-500`}>
            <stat.icon className="mx-auto" />
          </div>
          
          <div className="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white mb-3 group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-green-600 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-700">
            {stat.value}
          </div>
          
          <p className="text-lg sm:text-xl text-gray-600 dark:text-gray-400 font-semibold">{stat.label}</p>
        </motion.div>
      ))}
    </div>
  </div>
