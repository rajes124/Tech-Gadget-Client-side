...
<section className={`flex flex-col justify-center items-center min-h-screen pt-20 pb-12 text-center px-6 md:px-12 relative overflow-hidden ${
  ...
  >
    <div className="w-6 h-10 border-2 border-blue-400 rounded-full flex justify-center">
     ...
     {/* 3. Stats Section - Fixed Dark Mode Background */}
<section className="py-20 sm:py-24 px-6 md:px-12 bg-gradient-to-b from-blue-50/50 to-white dark:from-blue-950/40 dark:via-indigo-950/30 dark:to-gray-900">
  <div className="max-w-6xl mx-auto text-center">
    <motion.h2 
      initial={{ opacity: 0, y: 30 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      className="text-3xl sm:text-4xl md:text-5xl font-black mb-4 bg-gradient-to-r from-blue-600 to-green-600 bg-clip-text text-transparent drop-shadow-lg"
    >
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 max-w-5xl mx-auto">
      {[
        { icon: FaStar, value: stats.totalProducts.toLocaleString() || "1,200", label: "Products", color: "text-blue-500 dark:text-blue-400" },
        { icon: FaUsers, value: stats.totalUsers.toLocaleString() || "45,000", label: "Active Users", color: "text-green-500 dark:text-green-400" },
        { icon: FaTruck, value: `$${ (stats.totalExports || 2500).toLocaleString() }K`, label: "Total Exports", color: "text-purple-500 dark:text-purple-400" }
      ].map((stat, idx) => (
        <motion.div
          key={idx}
        ...
        >
          <div className={`text-5xl md:text-6xl mb-6 ${stat.color} drop-shadow-xl group-hover:scale-110 transition-transform duration-500`}>
            <stat.icon />
          </div>
          <div className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white mb-2 group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-green-600 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-500">
            {stat.value}
          </div>
          <p className="text-base md:text-lg text-gray-600 dark:text-gray-400 font-medium">{stat.label}</p>
          
          {/* Subtle glow effect on hover */}
          <div className="absolute inset-0 bg-gradient-to-t from-blue-500/10 to-transparent opacity-0 group-hover:opacity-100 rounded-3xl blur-3xl transition-opacity duration-700 -z-10"></div>
        </motion.div>
      ))}
    </div>
  </div>
