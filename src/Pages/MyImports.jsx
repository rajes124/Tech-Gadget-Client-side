...
const MyImports = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:4000/my-imports/${user?.uid}`)
      .then(res => res.json())
      .then(data => { setImports(data.filter(Boolean)); setLoading(false); })
  ...
  const handleReImport = async (id, available) => {
    ...
    try {
      ...
      if (res.ok) {
        setImports(prev => prev.map(item => item._id === id
    ...
    } catch (err) { console.error(err); alert("Failed to import"); }
  ...
  if (!imports.length) return (
    <div className="text-center mt-20 text-gray-600">
      ...
      <Link to="/" className="text-blue-500 mt-4 inline-block hover:underline">Browse Products</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto py-20 px-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {imports.map(item => (
          <motion.div key={item._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col">
            <img src={item.image} alt={item.name || item.productName} className="w-full h-48 object-cover rounded-xl mb-4" />
            ...
            <div className="mt-auto flex gap-2">
              <button onClick={() => handleRemove(item._id)} className="flex-1 px-4 py-2 bg-red-500 text-white rounded-xl hover:bg-red-600 transition">Remove</button>
              <button onClick={() => navigate(`/product/${item._id}`)} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition">See Details</button>
              <button
                onClick={() => handleReImport(item._id, item.availableQuantity || 0)}
                disabled={importingId === item._id || (item.availableQuantity || 0) <= 0}
                className={`flex-1 px-4 py-2 rounded-xl text-white transition ${ (item.availableQuantity || 0) > 0 ? "bg-green-500 hover:bg-green-600" : "bg-gray-400 cursor-not-allowed" }`}
              >
                {(item.availableQuantity || 0) > 0 ? importingId === item._id ? "Importing..." : "Re-Import" : "Out of Stock"}
              </button>
            </div>
          </motion.div>
        ))}
