  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <motion.div
        ...
        className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 items-center bg-gradient-to-br from-white/70 to-blue-50/50 backdrop-blur-md shadow-2xl rounded-3xl p-6 sm:p-10 border border-white/40 relative overflow-hidden"
      >
        <motion.img
          src={product.image}
          alt={product.name}
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 sm:p-8 w-full max-w-sm shadow-2xl border border-gray-200"
            >
              <h3 className="text-xl sm:text-2xl font-bold mb-3 text-gray-900">Import Product</h3>
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3">
                <button onClick={() => setShowModal(false)} className="px-5 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 transition w-full sm:w-auto">Cancel</button>
                <button
                  onClick={handleImport}
                  disabled={importQuantity <= 0 || importQuantity > product.availableQuantity || submitting}
                  className={`px-5 py-2 rounded-lg text-white font-semibold transition w-full sm:w-auto ${importQuantity <= 0 || importQuantity > product.availableQuantity || submitting ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 shadow-md"}`}
                >
